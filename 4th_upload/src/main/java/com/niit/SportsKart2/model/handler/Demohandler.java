...
public class Demohandler {
	...
	public String validateDetails(Register2 register2, MessageContext messageContext) {
		...
		if (register2.getMobilenumber() == null) {
			messageContext
					.addMessage(new MessageBuilder().error().source("mobilenumber").defaultText("Age cannot be Empty").build());
			status = "failure";
		}
		if (register2.getEnterpassword() == null) {
			messageContext
					.addMessage(new MessageBuilder().error().source("enterpassword").defaultText("Age cannot be Empty").build());
			status = "failure";
		}
		if (register2.getConfirmpassword() == null) {
			messageContext
					.addMessage(new MessageBuilder().error().source("confirmpassword").defaultText("Age cannot be Empty").build());
			status = "failure";
		}
		return status;
	}
